...
export function NodeDetail({
  ...
  return (
    <motion.aside
      ...
      className="glass relative flex min-h-0 flex-col overflow-hidden rounded-2xl"
    >
      <div className="flex items-start justify-between gap-2 border-b border-white/10 px-4 py-3">
        <div className="min-w-0">
          ...
          <h2 className="mt-1.5 truncate text-sm font-semibold text-slate-100">{node?.title ?? "Node"}</h2>
        </div>
        <button
          onClick={onClose}
      ...
      <div className="min-h-0 flex-1 overflow-auto scroll-thin p-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              {connections.map(({ ref, type, dir }) => (
                ...
                >
                  ...
                  <span className="font-mono text-[0.6rem] text-slate-500">{type}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {loading ? (
          ...
          <p className="text-sm text-slate-500">This node is no longer in the index.</p>
        )}
      </div>
    </motion.aside>
  );
